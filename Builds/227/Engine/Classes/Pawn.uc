...
var() localized  string[32] MenuName; //Name used for this pawn type in menus (e.g. player selection) 
var() localized  string[16] NameArticle; //article used in conjunction with this class (e.g. "a", "an")

var() byte VoiceType; //for speech
...
replication
{
	...
	unreliable if( Role==ROLE_Authority )
		ClientHearSound, ClientMessage, TeamMessage, ClientVoiceMessage;
	unreliable if( Role<ROLE_Authority )
		...
		{  
			if ( !bNoSpeak )
			{
				if ( (broadcasttype == 'GLOBAL') || !Level.Game.bTeamGame )
					P.ClientVoiceMessage(Sender, Recipient, messagetype, messageID, None);
				else if ( Sender.Team == P.PlayerReplicationInfo.Team )
					P.ClientVoiceMessage(Sender, Recipient, messagetype, messageID, Region.Zone);
			}
		}
...
//
intrinsic simulated event ClientHearSound ( 
	actor Actor, 
	int Id, 
	sound S, 
